...
export default function ProfileSection({ color }: { color?: string }) {
  ...
  const [profileColor, setProfileColor] = useState(COLORS[0].value)
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (profileRes.data) {
        ...
        setProfileColor(profileRes.data.color)
  ...
  const saveProfile = async () => {
    ...
    await supabase.from('profiles').update({ name: name.trim(), color: profileColor, avatar_url: avatarUrl }).eq('id', profile.id)
    setSaving(false)
    setSaved(true)
  ...
  if (loading) {
    return (
      <>
        <Header title="Профиль" color={color} />
    ...
    <>
      ...
      <main className="px-4 pt-5 pb-6 flex flex-col gap-5">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Нажмите чтобы сменить фото</p>
        </div>

        {/* Form */}
        <div className="rounded-2xl border p-5 flex flex-col gap-4" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium" style={{ color: 'var(--text)' }}>Имя</label>
            <input
              type="text"
              value={name}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm font-medium" style={{ color: 'var(--text)' }}>Цвет в календаре</label>
            <div className="flex gap-2 flex-wrap">
              {COLORS.map((c) => (
                <button
                  ...
                  style={{
                    background: c.value,
                    outline: profileColor === c.value ? `3px solid ${c.value}` : '3px solid transparent',
                    outlineOffset: '2px',
                  }}
                />
              ))}
          ...
          <button
            ...
            style={{ background: saved ? 'var(--success)' : (color ?? 'var(--primary)'), color: '#fff' }}
          >
              ...
              <div key={member.id} className="flex items-center gap-3">
                ...
                <p className="text-sm font-medium" style={{ color: 'var(--text)' }}>
                  ...
                  {member.id === profile?.id && (
                    <span className="ml-2 text-xs font-normal" style={{ color: 'var(--text-muted)' }}>(вы)</span>
                  )}
                </p>
              </div>
            ))}
        ...
        <button
          ...
          style={{ background: 'var(--surface)', color: 'var(--danger)', borderColor: 'var(--border)' }}
        >
          <LogOut size={16} />
          Выйти из аккаунта
        </button>
