...
export default function ShoppingPage() {
  ...
  return (
    <>
      <Header title="Покупки" subtitle="Выберите магазин" />
          ...
          <>
                  ...
                  <div>
                    ...
                    {itemCounts[store.id] ? (
                      <p className="text-xs mt-0.5" style={{ color: store.color }}>
                        {itemCounts[store.id]} {itemCounts[store.id] === 1 ? 'позиция' : 'позиций'}
                      </p>
                    ) : (
            ...
            {stores.length === 0 && !showAdd && (
              <p className="text-center text-sm py-4" style={{ color: 'var(--text-muted)' }}>
                Запустите SQL-схему в Supabase чтобы появились магазины по умолчанию
              </p>
            )}
          </>
        )}
